...
export const CROSS_COLORS_KEY = "crossColors"; // New constant for cross colors
...
export function setComponentData(
  componentNum,
  decibels,
  response,
  responseTime,
  ...
  if (!_.isUndefined(crossColors)) {
    store[key][CROSS_COLORS_KEY] = crossColors; // Store cross colors if provided
  }
...
export function setEncryptedMetadata(encryptedMetadata) {
  if (
    encryptedMetadata !== LocalStorageBackedStore.store[ENCRYPTED_METADATA_KEY]
  ) {
    // Reset state
