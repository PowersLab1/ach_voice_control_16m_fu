...
class Trial extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      invalid: false,
      readyToStart: false,
      dataSent: getDataSent(getEncryptedMetadata()),
    ...
    // For debugging: log the cross color being used
    console.log("Trial using cross color:", this.props.constantCrossColor);
    ...
    this.ratingsRaw = [];
    this.timestamps = [];
    this.crossColors = []; // New array to track cross colors for each trial

    // Also log trial decibel levels
    console.log("Trial decibel levels:", this.props.decibels);
  ...
  playStimulus = () => {
    ...
    // Record the cross color for this trial
    this.crossColors.push(this.props.constantCrossColor);
    ...
    // Log current trial info for debugging
    console.log(
      `Trial ${that.state.index + 1} - Decibel: ${amp}, Cross Color: ${that.props.constantCrossColor}`,
    );
  ...
  saveDataToStore() {
    this.props.dataHandler(
      ...
      this.ratingsRaw,
      this.timestamps,
      this.crossColors, // Pass the cross colors to the data handler
    );
  }
  ...
  recordResponse = (event) => {
    if (
      ...
      if (this.props.shouldRecordRatings) {
        ...
        // Update index and notify parent of progress
        this.setState({ index: this.state.index + 1 }, () => {
          if (this.props.onTrialProgress) {
            this.props.onTrialProgress(this.state.index);
          }
        });
      }
    } else {
  ...
  log_debug() {
    ...
    console.log("all cross colors: " + this.crossColors); // Added logging for cross colors
    console.log("fixation cross color: " + this.props.constantCrossColor);
  ...
  render() {
    ...
    if (this.state.invalid) {
      ...
      return this.props.trialCompleteRenderer(
        this.props.decibels,
        this.response,
        this.crossColors, // Add cross colors to the renderer if needed
      );
    }
        ...
        {this.state.trialStarted ? (
          <div className="Trial-stimulus">
            <VisualStimulus
              ...
              crossColor={this.props.constantCrossColor}
            />
          </div>
        ) : (
...
Trial.propTypes = {
  ...
  onTrialProgress: PropTypes.func,
};
