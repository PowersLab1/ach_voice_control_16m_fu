...
class LabJsWrapper extends Component {
  ...
  processLabJsData(labJsData) {
    ...
    for (let i = 1; i < labJsData.length; i++) {
      processedData.push(labJsData[i]);
    }
  ...
  componentDidMount() {
    ...
    window.addEventListener("message", function (event) {
      if (event.data.type === "labjs.data") {
        ...
        aws_saveTaskData(
          that.state.encryptedMetadata,
          that.packageDataForExport(parsedData),
        ).then(() => {
          if (that.surveyUrl) {
            that.setState({ link: that.surveyUrl });
          } else {
            aws_fetchLink(that.state.encryptedMetadata).then((link) =>
              that.setState({ link: link }),
            );
          }
        });
      }
    ...
    return (
      <div>
        <div
          ...
          style={{ visibility: this.state.sendingData ? "hidden" : "visible" }}
        >
        ...
        <div
          className="center"
          style={{ visibility: this.state.sendingData ? "visible" : "hidden" }}
        >
